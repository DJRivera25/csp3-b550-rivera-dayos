...
const AdminView = ({ products, fetchProducts }) => {
  ...
  const handleAddProductSave = async () => {
    ...
    try {
      ...
      // Append all fields except image normally
      ...
      if (newProductData.image && newProductData.image instanceof File) {
        formPayload.append("image", newProductData.image);
      }

      // Append isActive flag explicitly
      formPayload.append("isActive", "true"); // Note: form data values must be strings
      ...
      Swal.fire("Error", "An error occurred while saving the product. Check if the product already exists.", "error");
    } finally {
      setIsSaving(false);
    }
  };
